...
- **支持从本机拖入上传**：将本地文件/文件夹拖入当前 Bucket 目录即可加入上传队列。
- **应用内复制/剪切**：工具栏或 `Ctrl+Shift+C` / `Ctrl+X` / `Ctrl+Shift+V` 用于桶内对象复制与移动。
...
- 主窗口关闭策略：最小化到托盘 / 退出应用 / 每次询问（说明见感叹号悬停提示）。关闭确认框可勾选「记住此选择」。
- **开机自动启动**：可开启或关闭（系统登录时自动启动；桌面客户端可用）。
- **界面缩放**：默认 110%，范围 90%～125%，支持滑块与加减调节；保存在本机。
- 默认下载目录：每次询问 / 固定目录。
- 显示图片缩略图（仅本机）。
...
- 覆盖同名文件。
- **全局搜索**：默认返回条数；每天定时自动索引；登录后若索引过期则自动增量更新。
