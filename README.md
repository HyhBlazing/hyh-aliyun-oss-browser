...
用于浏览、管理阿里云 OSS 及兼容私有云对象存储：上传下载、预览、地址/二维码、多 AccessKey、传输队列与系统托盘等。
...
| 平台 | 附件示例 | 说明 |
| --- | --- | --- |
| Windows x64 | `*_x64-setup.exe` / `*_x64_en-US.msi` | 推荐 NSIS 安装包 |
| macOS Apple Silicon | `*_aarch64.dmg` | **仅 M 芯片（aarch64）**，不含 Intel |
| Linux | `*.deb` / `*.rpm` / `*.AppImage` | 以 Release 实际附件为准 |
...
- 桌面端可将对象拖到本地资源管理器 / 桌面（异常时回退到本机桌面）。
