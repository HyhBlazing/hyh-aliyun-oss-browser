...
angular.module('web').factory('ossSvs2', [
  ...
  function($q, $rootScope, $timeout, $state, Toast, Const, AuthInfo, settingsSvs) {
    ...
    function signatureUrl2(region, bucket, key, expires, xprocess, response) {
      ...
      return client.signatureUrl(key, opts);
    }
    ...
    function deleteFolder({ region, bucket, folder, marker, progCb, progress }) {
      ...
      const checkComplete = (resolve, reject) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        chunk.forEach((chunkFiles) => {
          ...
          client.deleteMulti(chunkFiles).then(() => {
            ...
            progress.errorCount += chunkFiles.length;
            errors.push(e);
            console.error('删除失败', e);
          }).then(() => {
            completedChunks++;

            if (completedChunks === chunk.length) {
              if (errors.length) {
                reject(errors);
              } else {
                resolve();
